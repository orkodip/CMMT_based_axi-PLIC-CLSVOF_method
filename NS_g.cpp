/*
NON-DIMENSIONAL NAVIER-STOKES SOLVER IN AXISYMMETRIC COORDINATES: ARRUFAT FORMULATION IS USED
...
TOP BOUNDARY -> OUTFLOW (ZERO PRESSURE) CONDITION
...
LEFT AND RIGHT BOUNDARY -> NEUMANN CONDITION
BOTTOM BOUNDARY -> DIRICHLET
TOP BOUNDARY -> UPDATED FROM CONTINUITY EQUATION

PRESSURE BOUNDARY CONDITIONS
LEFT, RIGHT AND BOTTOM BOUNDARY -> NEUMANN CONDITION
TOP BOUNDARY CELL -> DIRICHLET
 */
...
//NS::NS():MG_BICGSTAB(I*(J-1),100)	//BICGSTAB parameterized constructor
NS::NS():MG_FDQGMRES(8,I*(J-1),100)	//FDQGMRES parameterized constructor
{
	us=new double*[J+2];
	vs=new double*[J+2];
	D_x=new double*[J+1];
...
void NS::vel_bc()
{
	...
	for(int i=1;i<=I;i++)
	{
		...
		u[J+1][i]=u[J+1][i]-dt/dy*(u[J+1][i]-u[J][i]);	//top boundary: Orlanski extrapolation
		v[J+1][i]=v[J+1][i]-dt/dy*(v[J+1][i]-v[J][i]);
	}
...
void NS::P_bc()
{
	...
	for(int i=1;i<=I;i++)	//bottom and top boundary
	{
		...
		P[J+1][i]=P[J][i]=0.0;	//Dirichlet condition
	}
}
void NS::calc_curv()
{
...
void NS::mom()
{
	...
	for(int j=1;j<=J;j++)	//explicit calculation
	{
		for(int i=1;i<=I;i++)
		{
			us[j][i]=A_x[j][i]+dt*Oh/rho_np1[j][i]*D_x[j][i];
			vs[j][i]=A_y[j][i]+dt*Oh/rho_np1[j][i]*D_y[j][i];
		}
	}
}
void NS::calc_fc()
	...
	for(int j=1;j<=J;j++)	//calculation of face centered values excluding the boundaries
	{
		for(int i=1;i<=I;i++)
		{
			if(i<I)
			{
				...
				u_EW[j][i]=0.5*(us[j][i]+us[j][i+1])+rdx*rho_fp*(K_EW[j][i]-Bo*Grav.y*y_eff)*(F[j][i+1]-F[j][i]);
			}
			if(j<J)
			{
				...
				v_NS[j][i]=0.5*(vs[j][i]+vs[j+1][i])+rdy*rho_fp*(K_NS[j][i]-Bo*Grav.y*y_eff)*(F[j+1][i]-F[j][i]);
			}
		}
	}
}
void NS::Press()
{
	...
	for(int j=1;j<J;j++)	//excluding top boundary cells
	{
		for(int i=1;i<=I;i++)
		{
			...
			else if((i==1)&&(j>1)&&(j<(J-1)))	//left boundary
			{
			...
			else if((i==1)&&(j==(J-1)))	//top left corner cell
			{
				C0[cnt]=(j-2)*I+i-1;	//1,J-1 term
				A0[cnt]=-A_s;
				cnt++;
			...
			else if((j==(J-1))&&(i>1)&&(i<I))	//top boundary
			{
				...
				A0[cnt]=A_e+A_w+A_s+A_n;
				cnt++;
				C0[cnt]=(j-1)*I+i;	//i+1,J term
				A0[cnt]=-A_e;
				cnt++;
			}
			else if((j==(J-1))&&(i==I))	//top right corner cell
			{
				C0[cnt]=(j-2)*I+i-1;	//I,J-1 term
				A0[cnt]=-A_s;
				cnt++;
			...
			else if((i==I)&&(j>1)&&(j<(J-1)))	//right boundary
			{
				C0[cnt]=(j-2)*I+i-1;	//I,j-1 term
				A0[cnt]=-A_s;
				cnt++;
	...
	if(COUNT%10==0)
	{
		GMG2::setup(I/2,(J-1)/2,C0,R0,A0,C1,R1,A1);	//create the coarse grid coefficient matrices
		GMG2::setup(I/4,(J-1)/4,C1,R1,A1,C2,R2,A2);
		GMG2::setup(I/8,(J-1)/8,C2,R2,A2,C3,R3,A3);
		GMG2::setup(I/16,(J-1)/16,C3,R3,A3,C4,R4,A4);
...
void NS::update()
{
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			...
			u[j][i]=us[j][i]-0.5*rdx*((P[j][i+1]-P[j][i]-(K_EW[j][i]-Bo*Grav.y*y_eff_p)*(F[j][i+1]-F[j][i]))*rho_fp
						+(P[j][i]-P[j][i-1]-(K_EW[j][i-1]-Bo*Grav.y*y_eff_m)*(F[j][i]-F[j][i-1]))*rho_fm);
			rho_fp=2.0/(rho_np1[j][i]+rho_np1[j+1][i]);
			n.x=0.25*(Phi[j][i+1]+Phi[j+1][i+1]-Phi[j][i-1]-Phi[j+1][i-1])/dx;
			n.y=(Phi[j+1][i]-Phi[j][i])/dy;
			n=n.unit();
			...
			v[j][i]=vs[j][i]-0.5*rdy*((P[j+1][i]-P[j][i]-(K_NS[j][i]-Bo*Grav.y*y_eff_p)*(F[j+1][i]-F[j][i]))*rho_fp
						+(P[j][i]-P[j-1][i]-(K_NS[j-1][i]-Bo*Grav.y*y_eff_m)*(F[j][i]-F[j-1][i]))*rho_fm);
		}
	}
	for(int i=1;i<=I;i++)	//outflow boundary (based on continuity)
		v_NS[J][i]=v_NS[J-1][i]-dy/(CX[i]*dx)*(Xm[i]*u_EW[J][i]-Xm[i-1]*u_EW[J][i-1]);
}
void NS::continuity()
{
	double cont=0.0;
...
void NS::solve()
{
	...
	if((COUNT%10)==0) { continuity(); max_CFL(); mass_err(); }
}
void NS::max_CFL()
{
...
void NS::max_GFN()
{
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			gfn[0]=mu[j][i]/rho_np1[j][i]*comm*Oh;
			gfn[1]=0.5*(mu[j][i+1]/rho_np1[j][i+1]+mu[j][i]/rho_np1[j][i])*comm*Oh;
			gfn[2]=0.5*(mu[j+1][i]/rho_np1[j+1][i]+mu[j][i]/rho_np1[j][i])*comm*Oh;
