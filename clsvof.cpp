...
class CLSVOF:public MBASE
{
	...
	void reinit();	//LS reinitialization algorithm
	...
	void dis_den();	//discard density field after advection (USE ONLY IF ABSOLUTELY NECESSARY)
	void LS_bsd_trunc();	//LS based truncation of volume fraction field (USE ONLY IF ABSOLUTELY NECESSARY)
	public:
			CLSVOF(); ~CLSVOF();
			void ini(double ymc,double rm_o,double rm_i,double yfc,double rf);
...
void CLSVOF::ini(double ymc,double rm_o,double rm_i,double yfc,double rf)
{
	double **F_f,**Fm_in;	//volume fractions WRT the father drop and inner mother drop
	F_f=new double*[J+1];	//memory allocation
	Fm_in=new double*[J+1];
	for(int i=0;i<J+1;i++)
	{
		F_f[i]=new double[I+1];
		Fm_in[i]=new double[I+1];
	}
	INI ms_m_in(Xm,Ym,CX,CY,Fm_in,Phi,0.0,ymc,rm_i,rm_i);	//inner mother drop
	ms_m_in.VF();
	INI ms_m(Xm,Ym,CX,CY,F,Phi,0.0,ymc,rm_o,rm_o);	//outer mother drop
	ms_m.VF();
	INI ms_f(Xm,Ym,CX,CY,F_f,Phi,0.0,yfc,rf,rf);	//father drop
	ms_f.VF();
	for(int j=1;j<=J;j++)	//complete volume fractions field
		for(int i=1;i<=I;i++)
			F[j][i]+=F_f[j][i]-Fm_in[j][i];
	ms_m.reinit(F,Phi,tag);	//create the level set field
	mass_act=0.0;
	for(int j=1;j<=J;j++)
		for(int i=1;i<=I;i++)
			mass_act+=F[j][i]*CX[i]*dx*dy;
	updt_ghost(F);
	MBASE::vel_ini(0,65,513,643,-8.645e-3);	//is,ie,js,je,mag
	...
	for(int i=0;i<J+1;i++)	//memory release
	{
		delete[] F_f[i];
		delete[] Fm_in[i];
	}
...
//--------------------INITIALIZATION SCHEME (GEOMETRIC+ALGEBRAIC)--------------------------------------------------
	for(int j=1;j<=J;j++)	//initialize the LS and tag values and calculate LS for interfacial cells
	{
		for(int i=1;i<=I;i++)
		{
			if((abs(F[j][i])<=1e-3)||(abs(F[j][i]-1.0)<=1e-3))	//for single phase cells
			{
				tag[j][i]=0;
				Phit[j][i]=(2.0*F[j][i]-1.0)*gamma*1e6;
				continue;
			}
			tag[j][i]=1;
			a=recon_int(i,j,F[j][i],Phi,&N);
...
double CLSVOF::recon_int(int i,int j,double Fa,double **Phia,VECTOR *N)
{
	...
	if((!isfinite((*N).x)||!isfinite((*N).y))||((abs((*N).x)<=EPS)&&(abs((*N).y)<=EPS)))
	{
...
void CLSVOF::ls_complete(int t)
{
	...
	p_out<<"ZONE T=\""<<TIME<<"\", I="<<I+2<<", J="<<J+2<<", DATAPACKING=BLOCK, SOLUTIONTIME="<<TIME<<endl;
