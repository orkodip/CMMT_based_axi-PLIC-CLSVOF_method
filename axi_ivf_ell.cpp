//INITIALIZATION OF THE VOLUME FRACTIONS AND LEVEL SETS FOR AN ELLIPTIC INTERFACE IN AXISYMMETRIC COORDINATES
class INI	//initial volume fraction and level sets
{
	...
	public:
		INI(double *x,double *y,double *cx,double *cy,double **fa,double **phia,double xc,double yc,double A,double B);	//initialization of the input variables
		...
		void reinit(double **F,double **Phi,int **tag);	//determine the initial level set function from volume fractions
		void int_normal();	//export interface normal vector
};
INI::INI(double *x,double *y,double *cx,double *cy,double **fa,double **phia,double xc,double yc,double A,double B)
{
	X=x; Y=y; CX=cx; CY=cy;
	Fa=fa; Phia=phia;
	Xc=xc; Yc=yc; a=A; b=B;
...
//---------------------------DETERMINATION OF CELLS THROUGH WHICH INTERFACE PASSES-------------------
			...
			{
				if(abs(dist[temp]-rad[temp])<=EPS) continue;	//point coincides with the interface
...
//-------------------DETERMINING THE ORIENTATION OF THE INTERFACE AND CALCULATING THE INTERSECTION POINTS-------------
				for(int temp=0;temp<4;temp++)	//determining if the curve passes through the corner points
				{
					...
					{
						case 0:	//point a
							if(abs(b*b*pow((x0-Xc),2.0)+a*a*mYc*mYc-a*a*b*b)<=EPS)
							{
								x1=x0; y1=y0;
							...
							break;
						case 1:	//point b
							if(abs(b*b*pow((x0-Xc),2.0)+a*a*pow((y3-mYc),2.0)-a*a*b*b)<=EPS)
							{
								x1=x0; y1=y3;
							...
							break;
						case 2:	//point c
							if(abs(b*b*pow((x3-Xc),2.0)+a*a*pow((y3-mYc),2.0)-a*a*b*b)<=EPS)
							{
								x2=x3; y2=y3;
							...
							break;
						case 3:	//point d
							if(abs(b*b*pow((x3-Xc),2.0)+a*a*mYc*mYc-a*a*b*b)<=EPS)
							{
								x2=x3; y2=y0;
				...
				for(int temp=0;temp<4;temp++)	//determining the intersecting points between the curve and the cell faces
				{
					...
					if(Z<(-EPS)) continue;	//no intersection points on this face
					else if(abs(Z)<=EPS) Z=0.0;
					root1=(-B+sqrt(Z))/(2.0*A);  root2=(-B-sqrt(Z))/(2.0*A);	//calculate both the roots
					switch(temp)	//determine the required intersection point
					{
						case 0:	//west face
							if((root1>EPS)&&(root1<(y3-EPS)))	//root1 lies in the domain
							{
								x1=x0; y1=root1;	//(x2,y2) cannot lie in west face
							}
							else if((root2>EPS)&&(root2<(y3-EPS)))	//root2 lies in the domain
							{
								x1=x0; y1=root2;	//(x2,y2) cannot lie in west face
							}
							break;
						case 1:	//south face
							if((root1>x0)&&(root1<(x3-EPS)))	//root1 lies in the domain
							{
								if(x1==100.0) { x1=root1; y1=0.0; }
								else if(x2==100.0) { x2=root1; y2=0.0; }
								intg_flag=1;
							}
							else if((root2>x0)&&(root2<(x3-EPS)))	//root2 lies in the domain
							{
...
//-----------------CALCULATING THE VOLUME FRACTIONS-------------------------------
				if(intg_flag==1)	//volume fraction for horizontal configuration
				{
					...
					Z=(x1*x1*(y1-y0)+x2*x2*(y3-y2)-x0*x0*(y3-y0)+Xc*Xc*(y2-y1)+a*a*(y2-y1)-a*a/(3.0*b*b)*(pow((y2-mYc),3.0)
						-pow((y1-mYc),3.0))+vol_flag*a*Xc/b*((y2-mYc)*pow((b*b-pow((y2-mYc),2.0)),0.5)+b*b*asin((y2-mYc)/b)
						-(y1-mYc)*pow((b*b-pow((y1-mYc),2.0)),0.5)-b*b*asin((y1-mYc)/b)))/(2.0*CX[i]*(X[i]-X[i-1])*(Y[j]-Y[j-1]));
				}
				else if(intg_flag==0)	//volume fraction for vertical configuration
				{
					vol_flag=(y2>mYc)?1:-1;	//resolve the plus or minus ambiguity
					Z=(0.5*(y1*(x1*x1-x0*x0)+y2*(x3*x3-x2*x2)+mYc*(x2*x2-x1*x1))+vol_flag*b/a*(pow((a*a-pow((x1-Xc),2.0)),1.5)/3.0
						-pow((a*a-pow((x2-Xc),2.0)),1.5)/3.0+0.5*Xc*((x2-Xc)*pow((a*a-pow((x2-Xc),2.0)),0.5)+a*a*asin((x2-Xc)/a)
						-(x1-Xc)*pow((a*a-pow((x1-Xc),2.0)),0.5)-a*a*asin((x1-Xc)/a))))/(CX[i]*(X[i]-X[i-1])*(Y[j]-Y[j-1]));
				}
				if(vol_flag==1) Fa[j][i]=Z;
				else if(vol_flag==-1) Fa[j][i]=1.0-Z;
				//cout<<"i = "<<i<<", j = "<<j<<", x1,y1 = "<<x1<<","<<(y1+Y[j-1])<<"; x2,y2 = "<<x2<<","<<(y2+Y[j-1])<<", Fa = "<<Fa[j][i]<<endl;
			}	//end of else
...
{
	double V=0.0,Vt,err;
	Vt=Xc*M_PI*a*b;	//actual volume of ellipse
	for(int j=1;j<=J;j++)
		for(int i=1;i<=I;i++)
...
void INI::reinit(double **F,double **Phi,int **tag)
{
